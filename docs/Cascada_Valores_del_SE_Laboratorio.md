...
Cuatro cosas que el schema v1.0 hoy no cubre y que habrá que resolver al definir el ítem:

1. **Cuatro pruebas nuevas en `js/data/pruebas-sintaxis.js`**: `PRU-SINT-SE-02` (paradigma), `-03` (a sí mismo / el uno al otro), `-04` (supresión), `-05` (sustitución por *le/les*). `-01` se queda intacta.
2. **El campo `funcion` de una prueba no sirve para estas.** Hoy declara una etiqueta de `FUNC_ORAC`; lo que decide la prueba del paradigma no es una función, es una rama. Habrá que añadir un campo hermano (`valores`) o admitir `funcion: null` en las pruebas de tronco.
3. **Falta una causa de agramaticalidad.** El corpus de *se* trae un error clásico que las 19 causas actuales no saben nombrar: pluralizar el verbo de una impersonal con «a + persona» (*«Se han pagado a todos los proveedores»*, *«Se ayudaron a todos los afectados»* — ejercicios 56a y 14b de la fuente). `pasiva_refleja_intrans` no es eso. Propuesta: causa nº 20, `impersonal_pluralizada`, nivel `avanzado`, veredicto `agramatical` — ampliación retrocompatible, sin subir `schema_version`, con el mismo patrón documentado en §5 del schema para las tres causas de agosto.
4. **Un detalle del motor de cascadas.** El `dependsOn` de `MORPH_CASCADES` compara contra **un** valor (`{step, val}`); el subpaso de función depende de dos (reflexivo *o* recíproco). O se admite un array en `val`, o se duplica el paso. Es una línea de código, pero conviene decidirlo antes de escribirla.
