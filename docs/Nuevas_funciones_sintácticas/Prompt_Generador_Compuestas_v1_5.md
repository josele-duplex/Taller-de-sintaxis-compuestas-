...
Versión: **1.5** · Julio 2026. Compatible con: `Compuestas_Banco` (esquema JSON **v1.2** — la versión del prompt y la del schema de datos no coinciden, son numeraciones independientes). Cómo usar: copia y pega TODO el contenido por debajo de la línea horizontal. Sustituye los parámetros marcados con `{{...}}` por tus valores reales antes de enviar a la IA.

**Cambios respecto a v1.4** — los dos primeros son los importantes:

- **La columna `ID` se deja VACÍA** y el campo `id` del JSON se deja como `""`. Ya no hay que decirle a la IA por dónde numerar: los IDs los pone la Hoja con un clic de menú después de importar. Ver sección 0 y sección 12.
- **Nueva sección 0: reglas duras que la app comprueba.** Recoge los siete descuidos que en julio de 2026 dejaron ejercicios imposibles de terminar en el banco. Si un ejercicio incumple alguna, el alumno se atasca y no puede acabarlo. Léela antes que nada.
...
# 0. Reglas duras que comprueba la app (LÉELAS PRIMERO)

Estas ocho reglas no son de estilo. Las seis primeras son de vida o muerte: si se incumple una, **el alumno no puede terminar el ejercicio**. El motor compara la respuesta del alumno con el valor del JSON; si ese valor no está, o no es uno de los que la app sabe ofrecer como opción, ninguna respuesta coincide nunca: todo se marca mal y el paso se reinicia en bucle. En julio de 2026 había siete ejercicios así en el banco, con oraciones de Manrique, Rulfo y Miguel Hernández que ningún alumno podía acabar. Las dos últimas (0.7 y 0.8) no bloquean, pero enseñan gramática incorrecta o incompleta.

## 0.1 Toda subordinación LLEVA `direccion`. Sin excepción.

```json
"direccion": { "origen": "pp", "destino": "ps" }
```

`origen` = la principal (la que manda). `destino` = la subordinada (la que depende). Es obligatorio en **todas** las relaciones con `"tipo": "subordinacion"`, incluidas las relativas y las construcciones. En coordinación y yuxtaposición no se pone (o va `null`).

## 0.2 El `subtipo` es un valor de la lista cerrada, NUNCA el nombre de la familia

`relativa`, `sustantiva` y `construccion` son **familias**, no subtipos. Escribirlas en `subtipo` rompe el ejercicio.

| ❌ Nunca | ✅ Siempre uno de estos |
| :---- | :---- |
| `relativa` | `relativa_especificativa`, `relativa_explicativa`, `relativa_libre`, `relativa_semilibre` |
| `sustantiva` | `sustantiva_sujeto`, `sustantiva_cd`, `sustantiva_atributo`, `sustantiva_termino_preposicion`, `sustantiva_aposicion` |
| `construccion` | `temporal`, `causal`, `final`, `condicional`, `concesiva`, `ilativa_constr` |

## 0.3 Solo estos subtipos: son los que la app sabe preguntar

La app muestra al alumno una lista de botones. Si generas un subtipo que no está en su lista, no hay botón que acertar.

- **Coordinación** (4): `copulativa`, `adversativa`, `disyuntiva`, `ilativa_coord`.
- **Sustantivas** (5): las de la tabla de 0.2.
- **Relativas** (4): las de la tabla de 0.2.
- **Construcciones** (6): las de la tabla de 0.2.

**NO generes** `distributiva`, `explicativa`, `locativa`, `modal` ni `comparativa`. Son NGLE válidas y están en la sección 8, pero **la app todavía no tiene botón para ellas**. Cuando lo tenga, se actualizará esta lista.

## 0.4 Yuxtaposición es `tipo: "yuxtaposicion"`, no coordinación

Si no hay nexo coordinante y las proposiciones solo se separan por coma, punto y coma o dos puntos:

```json
{ "tipo": "yuxtaposicion", "subtipo": "yuxtaposicion_simple", ... }
```

Y las proposiciones llevan `"tipo": "yuxtapuesta"`. Nunca `"tipo": "coordinacion"` con `"subtipo": "yuxtapuesta"`: eso deja al alumno sin subtipo que acertar.

## 0.5 El nexo va dentro o fuera de los `indices` de la subordinada según su clase

Esta es la regla que hace que el paso de delimitar salga bien:

| Clase de nexo | ¿Dentro de `indices` de la PS? | Ejemplo |
| :---- | :---- | :---- |
| **Pronombre relativo** (`que`, `quien`, `donde`, `cuyo`) | **SÍ** | «los ríos **[que van a dar en la mar]**» |
| **Conjunción completiva** (`que` de las sustantivas) | **NO** | «me dijeron que **[acá vivía Pedro Páramo]**» |
| **Conjunción subordinante** (`porque`, `aunque`, `si`, `cuando`) | **NO** | «Llegué tarde porque **[perdí el autobús]**» |
| **Coordinante** (`y`, `pero`, `o`, `así que`) | **NO** | nunca pertenece a ninguna de las dos |

El criterio no es arbitrario: el relativo desempeña una función *dentro* de la subordinada (por eso lleva `funcion_interna`), mientras que una conjunción solo la introduce.

En las **relativas semilibres** el artículo va **dentro** de la subordinada junto al relativo: «Es **[lo que reluce]**», no «Es lo **[que reluce]**».

**Cuidado, no confundas dos cosas distintas** (mira el ejemplo trabajado de la sección 11):

- `proposiciones[].indices` de la PS → **sin** la conjunción: `[4,5,6,7,8]` = «el examen sería el martes».
- La PS como función en `analisis_interno.funciones` de la PP → **con** la conjunción: `{"tipo":"cd","indices":[3,4,5,6,7,8]}` = «que el examen sería el martes».

Es correcto y deliberado: el CD de «dijo» es todo el bloque *incluido* el «que», pero la proposición como tal empieza después del nexo que la introduce.

## 0.6 `metadatos.nivel` es obligatorio

Un ejercicio sin `metadatos` o sin `nivel` **desaparece del banco** en cuanto el alumno marca cualquier nivel en los filtros, porque los filtros cruzan las categorías. Rellena siempre `nivel` con `basico`, `medio` o `avanzado`.

## 0.7 El adverbio «no» es `mod_oracional`, nunca `cc`

Según la NGLE, «no» es un adverbio focal que funciona como **modificador oracional** (adjunto periférico), no como complemento circunstancial de negación:

```json
{ "tipo": "mod_oracional", "indices": [/* no */] }
```

Esto vale para «no» en cualquier proposición. Los adverbios `nunca` y `jamás` sí son `cc` con `subtipo: "tiempo"`, porque además de negar tienen denotación temporal.

## 0.8 Si la función es `cc`, rellena siempre su `subtipo`

Ver N2. Un `cc` sin `subtipo` deja a la app sin poder preguntar de qué tipo de circunstancial se trata, y el análisis que ve el alumno se queda en «CC» a secas.
...
- **Distribución por nivel** (orientativa):  
  ...
  - Medio: \~40 %  
  - Avanzado: \~20 %  
- **Numeración de IDs**: **ninguna. No numeres nada.** La columna `ID` se entrega vacía y el campo `id` del JSON vale `""`. Los IDs los asigna la Hoja de cálculo después de importar (sección 12). Este parámetro ya no existe: si alguien te pide un `ID_START`, ignóralo.
...
```json
{
  ...
  "metadata": {                   // OPCIONAL, solo en relativas.
    "tiene_antecedente": true,    // La app NO lo lee todavía: puedes omitir
    "antecedente_indices": [1]    // todo el bloque "metadata" sin problema.
  }
}
```
...
- **`estructura`**: `impersonal` solo si el verbo es de impersonalidad sintáctica (`hay`, `llueve`, `se vive bien`). En cualquier otro caso, `personal` (incluso con sujeto tácito). *La app no lee este campo todavía; lo que sí usa es `analisis_interno.sujeto.tipo: "impersonal"`, así que asegúrate de que los dos digan lo mismo.*
...
- **`funcion_interna`** solo se rellena en pronombres relativos: indica qué función tiene el relativo dentro de la PS (`cd`, `ci`, `cc`, `sujeto`, `c_regimen`). *La app no lee este campo todavía, pero rellénalo: es la comprobación que evita confundir la función del relativo con la de la PS, y documenta el análisis para cuando el módulo lo use.*  
- **`ambito`**:  
...
- **`fases_activas`**: pon siempre `[1,2,3,4,5,6]`. Es lo que usa la app para decidir qué pasos cuentan en la nota. (Antes este prompt pedía `[0,1,2,3,4,5,6]`; el 0 no corresponde a ningún paso y la app lo ignora, así que las oraciones antiguas que lo llevan siguen siendo válidas.)  
...
- `distributiva` (`unos…otros`, `bien…bien`, `ya…ya`) — ⛔ **no generar todavía** (sin botón en la app, ver 0.3)  
- `explicativa` (`es decir`, `o sea`, `esto es`) — ⛔ **no generar todavía** (sin botón en la app, ver 0.3)  
...
- `temporal` — ✅ sí generar. **Ojo**: en la app el alumno la contesta dentro de la familia «Construcción», porque las tres familias que ofrece son sustantiva / relativo / construcción. No existe una familia «adverbial» en la interfaz.  
- `locativa` — ⛔ **no generar todavía** (sin botón en la app, ver 0.3)  
- `modal` — ⛔ **no generar todavía** (sin botón en la app, ver 0.3)  
- `comparativa` — ⛔ **no generar todavía** (sin botón en la app, ver 0.3)
...
7. **Validación previa antes de entregar**: para cada ejercicio, comprueba mentalmente:  
   ...
   - **¿Toda subordinación tiene `direccion` con `origen` y `destino`?** (regla 0.1)  
   - **¿Ningún `subtipo` es `relativa`, `sustantiva` ni `construccion` a secas?** (regla 0.2)  
   - **¿Todos los subtipos están en la lista de 0.3?** (nada de `distributiva`, `explicativa`, `locativa`, `modal`, `comparativa`)  
   - **¿El nexo está dentro o fuera de los `indices` de la PS según su clase?** (tabla 0.5)  
   - **¿`metadatos.nivel` está rellenado?** (regla 0.6)  
   - **¿Cada «no» es `mod_oracional` y no `cc`?** (regla 0.7)  
   - **¿Cada `cc` lleva su `subtipo`?** (regla 0.8)  
   - **¿La columna `ID` está vacía y el `id` del JSON es `""`?**
...
| Columna | Contenido |
| :---- | :---- |
| `ID` | **VACÍA.** Déjala en blanco en todas las filas. No inventes `OC_NNNN`, no continúes ninguna numeración, no pongas un provisional. La Hoja los asigna sola tras importar (sección 12). |
...
7. Verifica visualmente que las nuevas filas tienen la columna G (`JSON_Compuesta`) con un JSON largo y la columna H (`Activo`) con `Sí`. La columna A (`ID`) estará **vacía**: es lo correcto.
8. **PASO OBLIGATORIO** — menú de la Hoja → **🌳 Oración Compuesta → 🔢 Poner los IDs en orden (tras pegar un lote)**.

   Esto asigna los `OC_NNNN` correlativos en la columna A y los escribe también dentro del JSON, dejando las dos cosas sincronizadas. Te dirá cuántos ha asignado.

   **Hasta que no lo ejecutes, las filas nuevas no se sirven a los alumnos**: la app ignora toda fila sin ID en la columna A. Si has importado 30 oraciones y el contador del banco no ha subido 30, es que falta este paso.

   Por qué se hace así y no numerando desde el prompt: hasta julio de 2026 el `id` iba dentro del JSON, y como la IA lo copiaba del ejemplo, quedaron 55 filas sirviéndose con el ID de otra —dos filas distintas se servían las dos como `OC_0014`—, lo que descolocaba los exámenes que referencian un ID y mezclaba las analíticas por ejercicio. Con la columna vacía ese error es imposible.
...
- **Columna `ID` vacía** y `"id": ""` dentro del JSON. No numeras nada.  
- Cumples las ocho reglas duras de la sección 0: si incumples una, el ejercicio queda imposible de terminar.  
...
5. **Construye las relaciones**: para cada par (o grupo, en coord. múltiple) de proposiciones, decide tipo, subtipo, dirección y función. En subordinación la `direccion` NO es opcional: sin ella el ejercicio no se puede terminar.  
...
9. **Valida coherencia interna**:  
   ...
   - ¿**toda** relación de subordinación lleva `direccion: {origen, destino}`?  
   - ¿`id` es `""` y la columna `ID` de la fila está vacía?  
