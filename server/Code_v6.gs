...
function onOpen() {
  ...
  mantenimiento.addItem('🔄 Reparar marcas Pas.Ref. / Imp.',      'menuRepararMarcasSE');
  mantenimiento.addItem('🔤 Reparar campos «segment» → «segmento»','menuRepararSegmentTypo');
  mantenimiento.addItem('🧹 Limpiar colores de auditoría',        'menuLimpiarColoresAuditoria');
  mantenimiento.addSeparator();
  mantenimiento.addItem('🔧 Reparar hoja Examenes_Config',        'menuRepararExamenesConfig');
  mantenimiento.addItem('🔧 Reparar hoja Alumnos_Resultados',     'menuRepararAlumnosResultados');
  ...
  tecnico.addItem('📋 Crear/actualizar cabeceras','crearCabeceras');
...
function configurarActivo() {
  const ss    = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_BANCO);
  if (!sheet) { SpreadsheetApp.getUi().alert('Hoja no encontrada.'); return; }
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) return;
  const range = sheet.getRange(2, COL_ACTIVO, lastRow - 1, 1);
  const rule  = SpreadsheetApp.newDataValidation()
    .requireValueInList(['Sí', 'No'], true).setAllowInvalid(false).build();
  range.setDataValidation(rule);
  SpreadsheetApp.getUi().alert('✅ Columna Activo (F) configurada con desplegable Sí/No.');
}
...
// ── Reparar JSONs rotos — escribe la versión corregida en la misma celda ──
function repararJsons() {
  const ss    = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_BANCO);
  if (!sheet) { SpreadsheetApp.getUi().alert('Hoja no encontrada.'); return; }
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) return;
  const data    = sheet.getRange(2, COL_JSON, lastRow - 1, 1).getValues();
  let fixed = 0, unfixable = 0;
  for (let i = 0; i < data.length; i++) {
    const raw = data[i][0] ? String(data[i][0]) : '';
    if (!raw.trim()) continue;
    try { JSON.parse(raw); continue; } catch(e) {} // already valid
    const parsed = safeParseJSON(raw);
    if (parsed) {
      sheet.getRange(i + 2, COL_JSON).setValue(JSON.stringify(parsed));
      fixed++;
    } else {
      unfixable++;
    }
  ...
  SpreadsheetApp.getUi().alert(
    ...
    '• ' + fixed + ' JSONs corregidos y guardados\n' +
    '• ' + unfixable + ' no recuperables (requieren revisión manual)'
  );
}

function validarJsons() {
  const ss    = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_BANCO);
  if (!sheet) return;
  const lastRow = sheet.getLastRow();
  const data    = sheet.getRange(2, 1, lastRow - 1, 6).getValues();
  let valid = 0, invalid = 0, fixable = 0;
  const badRows = [];
  for (let i = 0; i < data.length; i++) {
    const raw = data[i][COL_JSON - 1] ? String(data[i][COL_JSON - 1]) : '';
    if (!raw.trim()) continue;
    try { JSON.parse(raw); valid++; continue; } catch(e) {}
    const parsed = safeParseJSON(raw);
    if (parsed) { fixable++; badRows.push('  Fila ' + (i+2) + ' (recuperable): ' + String(data[i][0]).slice(0,40)); }
    else        { invalid++; badRows.push('  Fila ' + (i+2) + ' (ROTO):        ' + String(data[i][0]).slice(0,40)); }
  }
  const msg = '📊 Validación de JSONs:\n' +
    '✅ Válidos: ' + valid + '\n' +
    '🔧 Recuperables: ' + fixable + '\n' +
    '❌ Irrecuperables: ' + invalid + '\n\n' +
    (badRows.length ? badRows.slice(0,15).join('\n') : 'Todo OK');
...
function crearCabeceras() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  // Oraciones_Banco
  let sheet = ss.getSheetByName(SHEET_BANCO) || ss.insertSheet(SHEET_BANCO);
  const hdrs = ['Oracion_Texto','Sujeto','Verbo','Tipo de Sujeto','Estructura_JSON','Activo'];
  hdrs.forEach((h, i) => sheet.getRange(1, i+1).setValue(h).setFontWeight('bold'));
  // Alumnos_Resultados
  sheet = ss.getSheetByName(SHEET_RESULTS) || ss.insertSheet(SHEET_RESULTS);
  ['Fecha','Correo','Nombre','Modo_Juego','PIN_Examen','Tipo_Analisis',
   'Nota_Ponderada','Pts_Obtenidos','Pts_Disponibles','Total_Errores',
   'Sujeto_Pts','Funciones_Pts','NP_Pts','Sintagmas_Pts','Detalle_Errores_JSON'
  ].forEach((h, i) => sheet.getRange(1, i+1).setValue(h).setFontWeight('bold'));
  SpreadsheetApp.getUi().alert('✅ Cabeceras creadas/actualizadas en ambas hojas.');
}
...
function resumenEstadistico() {
  ...
  if (!sheet || sheet.getLastRow() < 2) {
    SpreadsheetApp.getUi().alert('No hay resultados todavía.'); return;
  }
  const data   = sheet.getRange(2, 1, sheet.getLastRow()-1, 15).getValues();
  const scores = data.map(r => parseFloat(r[6])).filter(n => !isNaN(n));
  if (!scores.length) { SpreadsheetApp.getUi().alert('No hay notas registradas.'); return; }
  const avg    = scores.reduce((a,b) => a+b, 0) / scores.length;
  const max    = Math.max(...scores);
  const min    = Math.min(...scores);
  const aprov  = scores.filter(s => s >= 5).length;
  SpreadsheetApp.getUi().alert(
    '📊 Resumen:\n' +
    '• Alumnos: ' + scores.length + '\n' +
    '• Media: ' + avg.toFixed(2) + '\n' +
    '• Máxima: ' + max + '\n' +
    '• Mínima: ' + min + '\n' +
    '• Aprobados: ' + aprov + ' (' + Math.round(aprov/scores.length*100) + '%)'
  );
}
...
// ════════════════════════════════════════════════════════════════════════
//  REPARACIÓN DE MARCAS DE PASIVA REFLEJA E IMPERSONALIDAD
// ════════════════════════════════════════════════════════════════════════
//  Detecta oraciones con "se" que carecen de Marca.Pas.Ref. o Marca.Imp.
//  y las clasifica automáticamente según el contenido del Sujeto:
//    · Sujeto = "---"               → Marca.Imp.   (oración impersonal)
//    · Sujeto explícito (no S.O.)   → Marca.Pas.Ref.
//    · Sujeto = "(S.O. ...)"        → SIN MARCA (verbo pronominal/reflexivo)
//
//  Actualiza tanto la columna E (añade el bloque "se") como la G
//  (añade la marca a funciones_presentes). Crea backup automático.

function menuRepararMarcasSE() {
  const ui = SpreadsheetApp.getUi();
  const r = ui.alert(
    '🔄 Reparar marcas Pasiva Refleja / Impersonal',
    'Voy a buscar oraciones que contengan «se» y que NO tengan etiquetada su marca correspondiente.\n\n' +
    'Para cada una, aplicaré esta lógica:\n\n' +
    '  • Sujeto = "---"                → Marca.Imp.\n' +
    '  • Sujeto explícito (no S.O.)    → Marca.Pas.Ref.\n' +
    '  • Sujeto = "(S.O. ...)"          → No es marca (verbo pronominal)\n\n' +
    'Las oraciones que no encajen claramente se marcarán en naranja para tu revisión.\n\n' +
    'Se creará una copia de respaldo de la hoja antes de modificar nada.\n\n' +
    '¿Continuar?',
    ui.ButtonSet.OK_CANCEL);
  if (r !== ui.Button.OK) return;
  try {
    const result = repararMarcasSE_();
    ui.alert('✓ Reparación completada',
      'Oraciones revisadas: ' + result.revisadas + '\n' +
      'Marca.Pas.Ref. añadida: ' + result.pasref + '\n' +
      'Marca.Imp. añadida: ' + result.imp + '\n' +
      'Sin marca (pronominales detectados): ' + result.pron + '\n' +
      'Marcadas para revisión manual: ' + result.dudosas + '\n\n' +
      'Hoja de respaldo: ' + result.backupName + '\n\n' +
      'Las oraciones modificadas están en verde claro. Las dudosas, en naranja. ' +
      'Recuerda redesplegar el GAS y limpiar el caché del script tras esto.',
      ui.ButtonSet.OK);
  } catch(e) {
    ui.alert('Error en la reparación', e.message, ui.ButtonSet.OK);
  }
}

function repararMarcasSE_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_BANCO);
  if (!sheet) throw new Error('Hoja Oraciones_Banco no encontrada');
  ...
  // Backup
  const fechaStr = Utilities.formatDate(new Date(), 'Europe/Madrid', 'yyyyMMdd_HHmm');
  const backupName = 'Oraciones_Banco_BackupSE_' + fechaStr;
  const backup = sheet.copyTo(ss);
  backup.setName(backupName);

  const COLOR_PASREF = '#DCFCE7'; // green
  const COLOR_IMP    = '#DBEAFE'; // blue
  const COLOR_DUDOSA = '#FEF3C7'; // amber

  // Lista de raíces que sugieren sujeto animado → NO es pasiva refleja
  const ANIMATE_ROOTS = [
    'herman','padre','madre','padres','hijo','hija','hijos','hijas',
    'abuel','niet','sobrin','tio','tia','prim','cuñad','suegr','yern','nuer',
    'amig','vecin','novi','espos','pareja','jef','colega','compañer',
    'niñ','bebé','beb[eé]s','joven','j[oó]venes','anciano','ancian',
    'persona','gente','hombre','mujer','muchach','chic','chico','chica','muchacho','muchacha',
    'alumn','profesor','maestr','docente','doctor','médico','medic','enfermer',
    'cantante','actor','actriz','m[uú]sico','escritor','autor','poeta','pintor',
    'jugador','atleta','deportista','futbolista','tenista','nadador',
    'capit[aá]n','soldado','militar','polic[ií]a','juez','testigo','abogad','fiscal',
    'cliente','paciente','víctima','viudo','viuda','huérfan',
    'turista','peregrin','excursionista','viajer','pasajer',
    'gato','perro','rat[oó]n','vaca','caballo','animal','pez','p[aá]jaro',
    'le[oó]n','tigre','oso','lobo','cordero','oveja','toro','cerdo',
    'rey','reina','pr[ií]ncipe','princesa','duque','duquesa','presidente',
    'director','gerent','empresari','vendedor','dependient',
    'señor','señora','don','doña','equipo','grupo','banda','familia',
    'soldados','marineros','peregrinos'
  ];
  const animateRe = new RegExp('\\b(' + ANIMATE_ROOTS.join('|') + ')[a-záéíóúñ]*\\b', 'i');
  // Personal pronouns as subject
  const pronounRe = /^\s*(él|ella|ellos|ellas|yo|tú|tu|nosotros|nosotras|vosotros|vosotras|usted|ustedes)\b/i;
  // Proper noun (capitalised first letter, no article preceding) — likely person
  const properNounRe = /^[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+(\s+[a-z]+\s+[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)?$/; // e.g. "Pedro", "Pedro y Luis", "Alfonso"
  const properNounMultiRe = /^[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+\s+y\s+[A-ZÁÉÍÓÚÑ]/; // "Pedro y Luis"

  const data = sheet.getDataRange().getValues();
  const seWordRe = /\b[Ss]e\b/;
  let revisadas = 0, pasref = 0, imp = 0, pron = 0, dudosas = 0;
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const activo = String(data[i][COL_ACTIVO - 1] || '').trim();
    if (activo !== 'Sí') continue;
    if (!seWordRe.test(oracion)) continue;

    const sujeto = String(data[i][1] || '').trim();
    const rawE = String(data[i][COL_JSON - 1] || '').trim();
    const rawG = String(data[i][COL_TAGS - 1] || '').trim();

    // Parse current funciones in E
    let parsedE = null;
    try { parsedE = JSON.parse(rawE); } catch(e) { continue; }
    if (!Array.isArray(parsedE)) {
      if (parsedE && typeof parsedE === 'object') parsedE = [parsedE];
      else continue;
    }
    const funcsE = parsedE
      .filter(it => it && typeof it === 'object' && it.función)
      .map(it => it.función);

    // If already has a marca, skip
    if (funcsE.indexOf('Marca.Pas.Ref.') >= 0 || funcsE.indexOf('Marca.Imp.') >= 0) continue;

    revisadas++;

    // ── Heurística por sujeto ──
    let nuevaMarca = null;
    let needsManualReview = false;

    if (sujeto === '---' || sujeto === '' || /\(?\s*Impersonal\s*\)?/i.test(sujeto)) {
      nuevaMarca = 'Marca.Imp.';
    } else if (/\(?\s*S\.\s*O\./i.test(sujeto)) {
      // Sujeto omitido — verbo pronominal/reflexivo, no añadir marca
      pron++;
      continue;
    } else if (animateRe.test(sujeto) || pronounRe.test(sujeto) || properNounRe.test(sujeto) || properNounMultiRe.test(sujeto)) {
      // Sujeto animado o nombre propio: probablemente reflexivo, recíproco o pronominal
      sheet.getRange(fila, 1, 1, 10).setBackground(COLOR_DUDOSA);
      dudosas++;
      continue;
    } else {
      // Sujeto inanimado explícito → pasiva refleja muy probable
      nuevaMarca = 'Marca.Pas.Ref.';
    }

    // Build new "se" block
    const isImp = (nuevaMarca === 'Marca.Imp.');
    const newBlock = {
      'segmento':   'se',
      'función':    nuevaMarca,
      'sintagma':   'SN',
      'naturaleza': 'Marca',
      'consejo':    isImp
        ? "El «se» aquí es marca de impersonalidad. La oración no tiene sujeto gramatical."
        : "El «se» aquí es marca de pasiva refleja. El sujeto paciente concuerda con el verbo.",
      'estructura': { 'se': 'Marca' }
    };

    // Insert "se" block at the start
    parsedE.unshift(newBlock);
    ...
    // Update column G
    let parsedG = {};
    try { if (rawG && rawG !== '{}') parsedG = JSON.parse(rawG); } catch(e) {}
    const fp = Array.isArray(parsedG.funciones_presentes) ? parsedG.funciones_presentes.slice() : [];
    if (fp.indexOf(nuevaMarca) < 0) fp.push(nuevaMarca);
    parsedG.funciones_presentes = fp.sort();
    if (!parsedG.tipo_oracion) parsedG.tipo_oracion = 'simple';
    if (!parsedG.predicado) parsedG.predicado = 'verbal';
    if (!parsedG.dificultad) parsedG.dificultad = 2;
    sheet.getRange(fila, COL_TAGS).setValue(JSON.stringify(parsedG));

    // Color row according to mark
    if (isImp) {
      sheet.getRange(fila, 1, 1, 10).setBackground(COLOR_IMP);
      imp++;
    } else {
      sheet.getRange(fila, 1, 1, 10).setBackground(COLOR_PASREF);
      pasref++;
    }
  }

  // Clear caches so the change takes effect
  try { CacheService.getScriptCache().removeAll(['morfologia_all']); } catch(e) {}

  return { revisadas, pasref, imp, pron, dudosas, backupName };
}

// ════════════════════════════════════════════════════════════════════════
//  REPARAR EXAMENES_CONFIG — limpia filas con datos desordenados
// ════════════════════════════════════════════════════════════════════════
//  Identifica filas donde los valores no encajan con sus columnas (p. ej.
//  porque fueron escritas con orden de columnas distinto al actual) y las
//  marca como 'cerrado' para que getExamConfig_ las ignore. Si la fila es
//  irrecuperable, se elimina. Tras esta función, todos los exámenes futuros
//  funcionarán correctamente porque buildRow_ ya escribe por nombre.

function menuRepararExamenesConfig() {
  const ui = SpreadsheetApp.getUi();
  const r = ui.alert(
    '🔧 Reparar hoja Examenes_Config',
    'Voy a inspeccionar la hoja Examenes_Config y a marcar como «cerrado» todas las filas cuyos datos no encajen con sus columnas (filas escritas en versiones anteriores).\n\n' +
    'También se eliminarán las columnas en blanco al final de la cabecera.\n\n' +
    'Esto NO afecta a futuros exámenes: los que crees a partir de ahora se escribirán correctamente.\n\n' +
    '¿Continuar?',
    ui.ButtonSet.OK_CANCEL);
  if (r !== ui.Button.OK) return;
  try {
    const result = repararExamenesConfig_();
    ui.alert('✓ Hoja reparada',
      'Filas marcadas como cerradas: ' + result.cerradas + '\n' +
      'Columnas vacías eliminadas: ' + result.colsLimpiadas + '\n\n' +
      'Ahora puedes crear nuevos exámenes con normalidad.',
      ui.ButtonSet.OK);
  } catch(e) {
    ui.alert('Error', e.message, ui.ButtonSet.OK);
  }
...
function repararExamenesConfig_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_EXAMS);
  if (!sheet) throw new Error('La hoja Examenes_Config no existe.');

  // 1) Eliminar columnas vacías al final de la cabecera
  let colsLimpiadas = 0;
  ...
  if (lastCol > 0) {
    ...
    for (let i = headers.length - 1; i >= 0; i--) {
      if (!headers[i] || String(headers[i]).trim() === '') trailingEmpty++;
      else break;
    }
    if (trailingEmpty > 0) {
      sheet.deleteColumns(lastCol - trailingEmpty + 1, trailingEmpty);
      colsLimpiadas = trailingEmpty;
    }
  }

  // 2) Asegurar todas las columnas estándar
  ensureSheetHeaders_(sheet, EXAM_HEADER);

  // 3) Detectar filas con datos desordenados y cerrarlas
  const col = getColMap_(sheet);
  const data = sheet.getDataRange().getValues();
  let cerradas = 0;
  for (let i = 1; i < data.length; i++) {
    const estado = String(data[i][col['Estado']] || '').trim();
    // Una fila está sana si Estado contiene 'activo', 'cerrado' o 'creando'
    // y PIN es numérico de 4+ dígitos
    const pin = String(data[i][col['PIN']] || '').trim();
    const validEstado = ['activo', 'cerrado', 'creando', ''].indexOf(estado) >= 0;
    const validPin = /^\d{4,}/.test(pin) || pin === '';
    if (!validEstado || !validPin) {
      // Fila corrupta — escribir 'cerrado' en la columna real
      sheet.getRange(i + 1, col['Estado'] + 1).setValue('cerrado');
      cerradas++;
    } else if (estado === 'activo') {
      // Si Estado='activo' pero Oraciones_JSON está vacío, también cerrar
      const oracJson = String(data[i][col['Oraciones_JSON']] || '').trim();
      if (!oracJson || oracJson === '[]') {
        sheet.getRange(i + 1, col['Estado'] + 1).setValue('cerrado');
        cerradas++;
      }
    }
  ...
  // 4) Limpiar caché
  try { CacheService.getScriptCache().removeAll(['exam_*']); } catch(e) {}

  return { cerradas, colsLimpiadas };
}

// ════════════════════════════════════════════════════════════════════════
//  REPARAR ALUMNOS_RESULTADOS — corrige labels desfasados sin perder datos
// ════════════════════════════════════════════════════════════════════════
//  Las versiones antiguas del código escribían valores por posición (orden
//  de RESULT_HEADER) en una hoja cuyas cabeceras tenían etiquetas distintas.
//  Esta función:
//    1) Hace una copia de seguridad de la hoja
//    2) Renombra las cabeceras al esquema correcto
//    3) Elimina columnas residuales sin uso

function menuRepararAlumnosResultados() {
  const ui = SpreadsheetApp.getUi();
  const r = ui.alert(
    '🔧 Reparar hoja Alumnos_Resultados',
    'Tu hoja tiene cabeceras antiguas (Modo_Juego, PIN_Examen, etc.) pero los datos en su interior corresponden al nuevo esquema (Grupo, Evaluacion, etc.).\n\n' +
    'Voy a:\n' +
    '  1) Crear una copia de seguridad de la hoja actual.\n' +
    '  2) Renombrar las cabeceras al esquema correcto.\n' +
    '  3) Eliminar las columnas vacías al final.\n\n' +
    'Los datos NO se mueven, solo se corrige su etiqueta.\n\n' +
    '¿Continuar?',
    ui.ButtonSet.OK_CANCEL);
  if (r !== ui.Button.OK) return;
  try {
    const result = repararAlumnosResultados_();
    ui.alert('✓ Reparada',
      'Cabeceras actualizadas: ' + result.renombradas + '\n' +
      'Columnas vacías eliminadas: ' + result.colsEliminadas + '\n' +
      'Filas conservadas: ' + result.filasConservadas + '\n\n' +
      'Copia de seguridad: ' + result.backupName,
      ui.ButtonSet.OK);
  } catch(e) {
    ui.alert('Error', e.message, ui.ButtonSet.OK);
  }
...
function repararAlumnosResultados_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_RESULTS);
  if (!sheet) throw new Error('La hoja Alumnos_Resultados no existe.');

  // Copia de seguridad
  const fechaStr = Utilities.formatDate(new Date(), 'Europe/Madrid', 'yyyyMMdd_HHmm');
  const backupName = 'Alumnos_Resultados_Backup_' + fechaStr;
  sheet.copyTo(ss).setName(backupName);

  // Mapa de renombres: header antiguo → header nuevo
  const RENAME_MAP = {
    'Modo_Juego':           'Grupo',
    'PIN_Examen':           'Evaluacion',
    'Tipo_Analisis':        'Examen',
    'Nota_Ponderada':       'PIN',
    'Pts_Obtenidos':        'Nota',
    'Pts_Disponibles':      'Completadas',
    'Total_Errores':        'Total_Oraciones',
    'Sintagmas_Pts':        'Elem_Fallados',
    'Detalle_Errores_JSON': '__delete__'  // marcador, no es header válido
  };

  // 1) Renombrar cabeceras
  const lastCol = sheet.getLastColumn();
  let renombradas = 0;
  let colsAEliminar = [];
  if (lastCol > 0) {
    ...
    headers.forEach((h, i) => {
      ...
      if (RENAME_MAP[orig]) {
        if (RENAME_MAP[orig] === '__delete__') {
          colsAEliminar.push(i + 1);  // 1-based para deleteColumn
        } else {
          sheet.getRange(1, i + 1).setValue(RENAME_MAP[orig]);
          renombradas++;
        }
      } else if (!orig) {
        // Columna sin cabecera al final
        colsAEliminar.push(i + 1);
      }
    });
  }

  // 2) Eliminar columnas marcadas (de derecha a izquierda para no cambiar índices)
  colsAEliminar.sort((a, b) => b - a);
  let colsEliminadas = 0;
  for (const c of colsAEliminar) {
    sheet.deleteColumn(c);
    colsEliminadas++;
  }

  // 3) Asegurar todas las columnas estándar
  const RESULT_HEADER = ['Fecha','Correo','Nombre','Grupo','Evaluacion','Examen','PIN',
                         'Nota','Completadas','Total_Oraciones',
                         'Sujeto_Pts','Funciones_Pts','NP_Pts','Elem_Fallados'];
  ensureSheetHeaders_(sheet, RESULT_HEADER);
  ...
  return {
    renombradas,
    colsEliminadas,
    filasConservadas: Math.max(0, sheet.getLastRow() - 1),
    backupName
  };
}
...
// ════════════════════════════════════════════════════════════════════════
//  REPARAR TYPO «segment» → «segmento»
// ════════════════════════════════════════════════════════════════════════
//  Algunas oraciones tienen el campo "segment" (inglés) en su JSON en lugar
//  de "segmento" (español). Esto rompe la fase 3 del análisis: los bloques
//  desaparecen porque el frontend no encuentra el texto del segmento. Esta
//  función recorre todas las oraciones activas y renombra el campo.

function menuRepararSegmentTypo() {
  const ui = SpreadsheetApp.getUi();
  const r = ui.alert(
    '🔤 Reparar typo "segment" → "segmento"',
    'Algunas oraciones tienen el campo "segment" en lugar de "segmento" en su JSON.\n' +
    'Esto provoca que la fase de funciones del predicado se quede vacía y haya que saltar la oración.\n\n' +
    'Voy a:\n' +
    '  1) Hacer una copia de seguridad de Oraciones_Banco\n' +
    '  2) Recorrer todas las oraciones activas\n' +
    '  3) Renombrar "segment" → "segmento" en su JSON estructural\n\n' +
    '¿Continuar?',
    ui.ButtonSet.OK_CANCEL);
  if (r !== ui.Button.OK) return;
  try {
    const result = repararSegmentTypo_();
    ui.alert('✓ Reparación completada',
      'Oraciones revisadas: ' + result.revisadas + '\n' +
      'Oraciones corregidas: ' + result.corregidas + '\n\n' +
      'Copia de seguridad: ' + result.backupName + '\n\n' +
      'Recuerda redesplegar el GAS y limpiar la caché.',
      ui.ButtonSet.OK);
  } catch(e) {
    ui.alert('Error', e.message, ui.ButtonSet.OK);
  }
...
function repararSegmentTypo_() {
  ...
  // Backup
  const fechaStr = Utilities.formatDate(new Date(), 'Europe/Madrid', 'yyyyMMdd_HHmm');
  const backupName = 'Oraciones_Banco_BackupSegment_' + fechaStr;
  sheet.copyTo(ss).setName(backupName);

  const data = sheet.getDataRange().getValues();
  let revisadas = 0, corregidas = 0;
  const COLOR_FIXED = '#DCFCE7'; // green
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    let parsed = null;
    try { parsed = JSON.parse(rawE); } catch(e) { continue; }
    if (!Array.isArray(parsed)) continue;

    let modified = false;
    for (let k = 0; k < parsed.length; k++) {
      const item = parsed[k];
      if (item && typeof item === 'object' && 'segment' in item && !('segmento' in item)) {
        item.segmento = item.segment;
        delete item.segment;
        modified = true;
      }
    }
    ...
    if (modified) {
      sheet.getRange(fila, COL_JSON).setValue(JSON.stringify(parsed));
      sheet.getRange(fila, 1, 1, 10).setBackground(COLOR_FIXED);
      corregidas++;
    }
  }
  ...
  return { revisadas, corregidas, backupName };
}
